...
- **业务导师：** 企微侧边栏常驻看板 + 独立 Web；一键知晓消红点，鼓励自愿提交带教记录（语音/文字），混元自动润色沉淀为年终绩效素材。
...
> 导师带教操作 ≤ 1 分钟（红点可一键知晓关闭；**鼓励**自愿记录带教核心，AI 润色为绩效素材换取积分）
...
| **举证式红点消除** | 导师**一键知晓**关闭红点（不阻塞）；**鼓励**自愿输入/录制一句带教核心（语音或文字），混元润色为「组织建设贡献描述」写入导师绩效素材库，带教变为获取绩效积分的加分项。 |
...
#### 5.3.1 举证式红点消除（激励替代强制，对抗导师懒惰）

导师点击 **[已阅，知晓 ✓]** 即可一键关闭红点，工作流不阻塞。
此后系统展示激励牌，**鼓励**导师自愿记录带教要点：

- 输入 **一句带教核心**（文字），或点击 🎤 录制 **≤30 秒语音**（模拟 ASR 识别后自动填入文字）
- 提交后混元-Pro 润色为标准化 **「组织建设贡献描述」**，写入 `mentor_contribution_logs`
- 素材可直接用于导师年终绩效/晋升材料（带教从负担变为 **刷绩效积分**）
...
| **管理流产（自欺红点消灭术）** | 一键消红点，导师 KPI 压力下空点，小抄进垃圾桶 | **举证式激励消红点**：一键知晓不阻塞 + 自愿记录换积分 + 混元生成 **绩效贡献素材**（利益驱动根治空点） |
